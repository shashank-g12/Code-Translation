...
def exec_sample(
    ...
    if target_lang =="Python":
        ...
        try:
            subprocess.run(f"python3 -m py_compile temp_dir/{problem['id']}-{problem['language']}-{target_lang}-{completion_id}/{problem['id']}.py", check=True, capture_output=True, shell=True, timeout=30)#change timeout
            ...
            for i in range(len(test_io)):
                ...
                if(stdout.strip()==f_out.strip()):
                    count+=1
                    if (count==len(test_io)):
                        stat.value = _SUCCESS
                    continue

                else:#may put continue and break seperately for qualitive analysis 
        ...
        with open(f"temp_dir/{problem['id']}-{problem['language']}-{target_lang}-{completion_id}/{problem['id']}.java", "w") as f:
            ...
            code= re.sub(r'public\s*class\s*[^{]+', r'public class ' + problem["id"], code)
            f.write(code)
...
# deatiled analysis of reasons
# concurrent features -terminate and alive 
